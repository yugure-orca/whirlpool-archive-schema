use crate::schema::event::{WhirlpoolEventBlock, WhirlpoolEvent};
...
#[test]
fn test_deserialize() {
    let deserialized: WhirlpoolEventBlock = serde_json::from_str(TEST_DATA).unwrap();
    ...
    match &deserialized.transactions[0].events[0] {
        WhirlpoolEvent::Traded(payload) => {
            ...
            assert_eq!(payload.whirlpool, "8SwU6NXuWbpuUR9FgpuAHFNjUpF28QyF7im3ZyKq7Ni4");
        }
